...
The entire R Notebook for the tutorial can be downloaded [here](/tutorials/dimred/dimred.qmd).  If you want to render the R Notebook on your machine, i.e. knitting the document to html or a pdf, you need to make sure that you have R and RStudio installed and you also need to download the [**bibliography file**](/assets/bibliography.bib) and store it in the same folder where you store the Rproj file. <br><br>
...
<p style='margin-top:1em; text-align:center'>
Click [here](/tutorials/dimred/dimred.qmd)^[If you want to render the R Notebook on your machine, i.e. knitting the document to html or a pdf, you need to make sure that you have R and RStudio installed and you also need to download the [**bibliography file**](/assets/bibliography.bib) and store it in the same folder where you store the Rmd file.] to download the **entire R Notebook** for this tutorial.
</p>
...
notebook_path <- "interactive-notebooks/notebooks/dimred_cb.ipynb"
...
pcadat %>%
  ...
  geom_abline(
    intercept = 0, slope = eigen$vectors.scaled[1, 1], colour = "orange",
...
pcadat %>%
  ...
  geom_hline(yintercept = 0, color = "red", linetype = "dotted") +
  geom_abline(
    intercept = intercept, slope = slope, color = "orange",
    linetype = "dashed", size = 1
  ) +
...
pcadat %>%
  ...
  geom_segment(aes(x = 0, y = 0, xend = 0.7282191, yend = 0.6853444),
    size = 1.5,
...
pcadat %>%
  ...
  geom_abline(
    intercept = 0, slope = -1 / eigen$vectors.scaled[1, 1], colour = "blue",
    linetype = "dashed", size = 1
  ) +
  coord_cartesian(xlim = c(-2, 2), ylim = c(-2, 2))
```
...
pcadat_rotated <- data.frame(
  Feat1 = pcadat$Feat1 * cos(radians) - pcadat$Feat2 * sin(radians),
...
```{r}
surveydata <- base::readRDS("tutorials/dimred/data/sud.rda", "rb")
# inspect
...
Schweinberger, Martin. 2025. *Introduction to Dimension Reduction Methods with R*. The University of Queensland, Australia. School of Languages and Culture. url: www.ladal.edu.au/tutorials/dimred/dimred.html  (Version 2025.04.01).

```
@manual{schweinberger2025dimred,
  author = {Schweinberger, Martin},
  title = {Introduction to Dimension Reduction Methods with R},
  note = {www.ladal.edu.au/tutorials/dimred/dimred.html},
    year = {2025},

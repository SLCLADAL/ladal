...
This tutorial shows how to work with tables and how to tabulate data in R. A more advanced and detailed but also truly excellent and highly recommended resource on processing data in R is @wickham2016r [see also @wickham2019welcome]. Alternative but also very useful resources are @stander2017enthusing and @estrellado2020data.

![](/images/gy_chili.png){ width=15% style="float:right; padding:10px" }

This tutorial is intended for beginners and intermediate users of R with the aim of showcasing how to work with and process tabulated data (tables) using R. The aim is not to provide a fully-fledged analysis but rather to show and exemplify selected useful functions and methods associated with tabular data. 
...
<p style='margin-top:1em; text-align:center'>
Click [here](/tutorials/table/table.qmd)^[If you want to render the R Notebook on your machine, i.e. knitting the document to html or a pdf, you need to make sure that you have R and RStudio installed and you also need to download the [**bibliography file**](/assets/bibliography.bib) and store it in the same folder where you store the Rmd file.] to download the **entire R Notebook** for this tutorial.
</p>
...
notebook_path <- "interactive-notebooks/notebooks/table_cb.ipynb"
...
This tutorial is based on R. If you have not installed R or are new to it, you will find an introduction to R and more information on how to use R [here](/tutorials/intror/intror.html). For this tutorials, we need to install certain *packages* from an R *library* so that the scripts shown below can be executed without errors. Before turning to the code in the sections below, please install the packages by running the code below this paragraph. If you have already installed the packages mentioned below, then you can skip ahead ignore this section. To install the necessary packages, simply run the following code. Don't worry if it takes some time - it may take up to 5 minutes to install all of the packages.
...
Tibbles are the tidyverse equivalent of data frames. They offer new functions and possibilities of handling and inspecting the data. 

# Loading tables into R{-}

There are several different functions that allow us to read tabulated data into R. In our case, we use the `readRDS` function which loads R data sets. 

```{r lt1, message=FALSE, warning=FALSE}
# load data with read.delim
mytable <- base::readRDS("tutorials/table/data/mld.rda", "rb")
```
...
# load data with read.delim
tab1 <- read.delim("tutorials/table/data/mlrdata.txt",
    sep = "\t", header = TRUE
)
tab2 <- read.table("tutorials/table/data/mlrdata.txt", header = TRUE)
```
...
# load data
exceldata <- openxlsx::read.xlsx("tutorials/table/data/testdata1.xlsx",
    sheet = 1
)
```
...
# load data
excelcomp <- readxl::read_excel(here::here("tutorials/table/data", "testdata1.xlsx"), sheet = 1)
```
...
We could, for example, load data and then capitalise the column names and then group the data by status and attraction and then get the mean of money spent, deleting all observations except for the first one. A more formal way to write this would be:

> load %>% capitalise %>% group %>% summarise.
...
# inspect summarised data
pipeddata
```

The pipe has worked, and we get the resulting summary which shows the mean of the money spent based on  Attraction and Status.

## Selecting and filtering{-}

Among the most frequent procedures in data processing is selecting certain columns or extracting rows based on variable levels. In the tidyverse, this is done by using the `select` and `filter` functions. While `select` allows you to extract columns, `filter` is used to extract rows, e.g. to get only observations that have a certain feature. Have a look at the example below.
...
Changing and adding data is done with the `mutate` function. The `mutate` function requires that we specify a column name. If we use the same name as the column we are changing, then we change the column, but if we specify another column name, then a new column is created. 

We will now create a new column (Spendalot) in which we encode if the person has spent a lot of money (100 AUD or more) on the present or not (less than 100 AUD).
...
The table now has a new column (Spendalot), because we specified a column name that did not exist yet - had we written `dplyr::mutate(money = ifelse(money >= 100, "Alot", "Alittle"))` then we would have changed the money column and replaced the money values with the labels *Alot* and *Alittle*.

## Renaming columns{-}

Oftentimes, column names are not really meaningful or even incoherent, which makes it harder to wrap your head around what the values in a column refer to. The easiest way around this is rename columns which is, fortunately, very simple in the tidyverse. While the column names of our example table are already meaningful, I want to capitalise the first letter of each column name. This can be done as follows.
...
Summarizing is particularly useful when we want to get summaries of groups. We will modify the example from above and extract the mean and the standard deviation of the money spent on presents by relationship status and whether the giver was attracted to the giv-ee. 
...
We can re-convert the wide data into a long format using the `gather` function. 
...
Schweinberger, Martin. 2025. *Handling tables in R*. Brisbane: The University of Queensland. url: https://ladal.edu.au/tutorials/table/table.html (Version 2025.04.01).


```
@manual{schweinberger2025table,

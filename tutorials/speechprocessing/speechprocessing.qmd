...
params:
  ...
  url:         "https://ladal.edu.au/tutorials/speechprocessing/speechprocessing.html"
  institution: "The Language Technology and Data Analysis Laboratory (LADAL), The University of Queensland, Australia"
  description:       "This tutorial introduces speech processing in R, covering automatic speech recognition (ASR), audio file handling, feature extraction from speech signals, and the use of R packages for phonetic analysis. It is aimed at researchers in phonetics, linguistics, and digital humanities who want to apply computational methods to spoken language data."
  keywords:    "Whisper ASR, speech recognition, automated speech recognition tutorial, audio processing, speech feature extraction, phonetic analysis, speech-to-text R workflow, computational linguistics speech data"
  doi:         "10.5281/zenodo.19424881"
---
...
# Use your own file (path is relative to this .qmd):  
...
```{r load_audio, eval=TRUE}  
# Primary: your own WAV file at data/sample.wav (relative to this .qmd)  
# Must be 16-bit mono 16 kHz — see the format callout below  
audio_path <- "data/sample.wav"  

# Fallback: use the built-in JFK sample shipped with audio.whisper  
# if the file above does not exist  
if (!file.exists(audio_path)) {  
  audio_path <- system.file(package = "audio.whisper", "samples", "jfk.wav")  
  message("Using built-in JFK sample; place your own file at data/sample.wav to override.")  
}  
...
# Compute a safe upper frequency limit strictly below Nyquist (samp.rate / 2).  
# seewave::spectro errors if flim reaches Nyquist exactly.  
nyquist_khz <- wav@samp.rate / 2000        # Nyquist in kHz  
safe_upper  <- min(8, nyquist_khz * 0.95)  # cap at 8 kHz, or 95% of Nyquist  

# Narrow-band spectrogram (good for pitch/formants)  
seewave::spectro(wav,  
                 flim = c(0, safe_upper),  # frequency limit in kHz  
                 main = "Spectrogram",  
...
```{r download_model, eval=TRUE}  
# Download models — specify each name. Each model is cached on first  
# download and re-used from cache on subsequent renders.  
whisper_download_model("base")      # 142 MB — used for basic transcription  
whisper_download_model("small")     # 466 MB — used for VAD and diarisation demos  

# Optional: much larger, best accuracy/speed trade-off. Uncomment to fetch (~1.5 GB).  
# whisper_download_model("large-v3-turbo")  
```  
  
## Basic Transcription {-}  
  
```{r transcribe_basic, eval=TRUE}  
# Load the model  
...
```{r trans_vad_internal, eval=TRUE}  
model <- audio.whisper::whisper("small")  
...
The example below is set to `eval=FALSE` because it requires a non-English audio file (`audio_path_dutch`) that is not shipped with the tutorial. To run it yourself, place a WAV file in an accessible location, assign its path to `audio_path_dutch`, and change `eval=FALSE` to `eval=TRUE`.  
:::  
...
The **general** pipeline in the field is:  
  
```  
Audio → VAD → Speaker Embedding Extraction → Clustering → Label Assignment  
```  
  
That is what dedicated tools such as `pyannote.audio` do. `audio.whisper` implements something simpler and more restrictive, described below.  
  
## What `audio.whisper` Actually Does {-}  
  
`audio.whisper`'s `diarize = TRUE` argument does **not** perform embedding-based diarisation. It performs a **channel split**: it assumes the input is a stereo (two-channel) file in which each channel corresponds to a different speaker (as in interview recordings where each participant has their own microphone), and labels each segment by which channel it originated from.  
  
This means:  
  
- The input WAV **must be stereo** (2 channels). Mono files cannot be diarised with `audio.whisper` and will produce a misleading "input needs to be a 16-bit .wav file" error.  
- It cannot identify more than two speakers.  
- It cannot separate speakers who share a single channel (typical of most field recordings, dictaphone recordings, or single-microphone conversations).  
  
For real embedding-based speaker diarisation on mono audio — the more common research case — use [pyannote.audio](https://github.com/pyannote/pyannote-audio) (Python) and match its speaker turns against Whisper's segment timestamps.  
  
## Running Channel-Based Diarisation with `audio.whisper` {-}  
  
::: {.callout-warning}  
## These chunks are not executed  
  
The chunks below are set to `eval=FALSE` because the tutorial's sample audio is mono. To run them yourself, provide a stereo WAV file (e.g. an interview with a separate mic per speaker) as `audio_path_stereo` and change `eval=FALSE` to `eval=TRUE`.  
:::  
  
```{r diarise, eval=FALSE}  
model <- audio.whisper::whisper("small")  
  
# audio_path_stereo must be a 2-channel WAV file  
trans_diarised <- predict(model,  
                          newdata   = audio_path_stereo,  
                          language  = "en",  
                          diarize   = TRUE,    # channel-based diarisation  
                          vad       = TRUE,  
                          n_threads = 4)  
...
data.frame(  
  ...
  flextable::fontsize(size = 11, part = "header") |>  
  flextable::align_text_col(align = "left") |>  
  flextable::set_caption(caption = "Example of channel-based diarised output. Speaker labels correspond to the left (0) and right (1) audio channels.") |>  
  flextable::border_outer()  
```  
...
speaker_0_text <- trans_diarised$data |>  
  dplyr::filter(speaker == "0") |>  
  dplyr::pull(text) |>  
  paste(collapse = " ")  
```  
  
::: {.callout-tip}  
## Limitations of `audio.whisper` Diarisation  
  
`audio.whisper`'s diarisation is convenient but strictly limited:  
  
- It is a **channel split**, not embedding-based clustering — it works only on stereo (2-channel) input  
- It cannot separate speakers who share a channel  
- It caps out at two speakers regardless of the actual number of participants  
- Cross-talk between channels bleeds into both speaker labels  
  
For genuine embedding-based diarisation — required for mono audio, more than two speakers, or single-mic recordings — use [pyannote.audio](https://github.com/pyannote/pyannote-audio) (Python) and match its speaker turns against Whisper's segment timestamps.  
:::  
...
```{r save_trans, eval=TRUE}  
# Ensure the outputs directory exists before writing to it  
...
::: {.callout-warning}  
## Code in this section is not executed — pending revision  
  
The code below calls `seewave::formant()`, but `seewave` does not export a `formant()` function. Formant extraction in R requires a different package: `wrassp::forest()` for time-based tracking, `phonTools::findformants()` for single-point estimation, or Praat integration via `rPraat` / `PraatR`. The three code chunks in this section are set to `eval=FALSE` pending a proper rewrite. The surrounding explanation of formants, F1/F2, and the vowel space remains accurate and worth reading.  
:::  

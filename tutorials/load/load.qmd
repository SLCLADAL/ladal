...
The data we work with comes in many formats and types. Therefore, this tutorial shows how you can load and save different types of data when working with R and includes a brief look at how to generate data in R.
...
To be able to follow this tutorial, we suggest you check out and familiarise yourself with the content of the following **R Basics** tutorial:<br>
</p>
...
This tutorial is based on R. If you have not installed R or are new to it, you will find an introduction to and more information on how to use R [here](/tutorials/intror/intror.html). For this tutorials, we need to install certain *packages* from an R *library* so that the scripts shown below can be executed without errors. Before turning to the code below, please install the packages by running the code below this paragraph. If you have already installed the packages mentioned below, then you can skip ahead ignore this section. To install the necessary packages, simply run the following code. Don't worry if it takes some time - it may take up to 5 minutes to install all the packages.
...
<br>
If you have a different setup, you will need to adjust the path to the data in order for the tutorial to function correctly on your own computer. It is worth mentioning that the `here` function is utilised to create paths that originate from the directory where the Rproj is located.</p>
...
* [mydoc.docx](/tutorials/load/data/mydoc.docx)
...
A common data type when working with tabulated data is a comma separated file (csv). To load such files, we can use the `read.csv` function as shown below.
...
The data is not spectacular and consists of a table with 2 columns (`Variable1`, and `Variable2`).
...
To save a data set as a csv on your computer (here it is saved within the `data` folder within the folder where the Rproj is located), use the `write.csv` function.
...
To load text data into R (here in the form of a txt file), we can use the `scan` function. Reading in texts using the `scan` function will result in loading vectors of strings where each string represents a separate word.
...
In contrast, the `readLines` function will read in complete lines  and result in a vector of strings representing lines (if the entire text is in 1 line, the the entire text will be loaded as a single string).
...
When working with linguistic or textual data, it’s not uncommon to encounter data stored in Microsoft Word (.docx) format — for example, interview transcripts, survey responses, or annotated documents. The `officer` package provides a convenient way to read and extract content from these documents directly in R.


To load docx-files, you will need:

+ The `officer` package (for reading .docx files)  

+ The `here` package (for file paths, optional but recommended)  

You can read a Word document into R using `read_docx()` and then summarize its contents using `docx_summary()` (both from the `officer` package):

```{r officer1}
doc_object <- officer::read_docx(here::here("tutorials/load/data/mydoc.docx"))
# summary
content_summary <- docx_summary(doc_object)
```

This will return a data frame where each row corresponds to an element in the document (e.g., a paragraph, heading, table cell, etc.). The summary includes information such as the element type, its textual content, style, and position in the document.
...
This makes it easy to filter, analyze, or clean text based on structure (e.g., only headings, only paragraphs, etc.).

As a tip: for most corpus tasks, you’ll want to filter out non-paragraph elements or extract specific text styles (e.g., Heading 1, Normal) depending on your analytical goals.


# R data objects {-}

When working withR in RStudio, it makes sense to save data as R data objects as this requires minimal storage space and allows us to load and save data very quickly. R data objects can have any format (structured, unstructured, lists, etc.). Here, we use the `readRDS` function to load R data objects (which can represent any form or type of data). 
...
You can load all types of data discussed above from the web. The only thing you need to do is to change the path. Instead of defining a path on your own computer, simply replace it with a URL within the `url` function and the additional argument `"rb"`.
...
Next, we create a correlation matrix. Here, we will create 4 variables. For each of these variables, we have to determine  how strongly each variable should be correlated with each other variable. The diagonal values are 1 as each variable correlates perfectly with itself.
...
If all variables should have the same mean, then we only need to provide a single value, but we need to provide 4 values if we want the variables to have different means.
...
If you want to generate numeric data, then this is all you need to do. If you want to generate categorical variables, however, we need to convert these numeric values into factors. In the example below, we convert all values higher than 1 (the mean) into one level, and all other values into a second level. 
...
If you need to generate many sentences that have a standard format, you can use the paste function.
...
Schweinberger, Martin. 2025. *Loading, saving, and generating data in R*. Brisbane: The University of Queensland. URL: https://ladal.edu.au/tutorials/load/load.html (Version 2025.04.01).


```
@manual{schweinberger2025loadr,

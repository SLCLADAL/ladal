...
The entire R Notebook for the tutorial can be downloaded [here](/tutorials/pdf2txt/pdf2txt.qmd).  If you want to render the R Notebook on your machine, i.e. knitting the document to html or a pdf, you need to make sure that you have R and RStudio installed and you also need to download the [**bibliography file**](/assets/bibliography.bib) and store it in the same folder where you store the Rmd file.
</p>
...
notebook_path <- "interactive-notebooks/notebooks/pdf2txt_cb.ipynb"
...
[pdf0](/tutorials/pdf2txt/data/PDFs/pdf0.pdf), [pdf1](/tutorials/pdf2txt/data/PDFs/pdf1.pdf), [pdf2](/tutorials/pdf2txt/data/PDFs/pdf2.pdf), and [pdf3](/tutorials/pdf2txt/data/PDFs/pdf3.pdf).

**R and RStudio set up**

1. Download the [RNotebook](/tutorials/pdf2txt/pdf2txt.Rmd) and save it in the folder you have just created
2. Open RStudio
...
The pdf we will convert is a [pdf of the Wikipedia article about corpus linguistics](/tutorials/pdf2txt/data/PDFs/pdf0.pdf). The first part of that pdf is shown below.

![](/images/pdf0.png){ width=100% }
...
```{r pconv02, echo=FALSE, eval = T, message=FALSE, warning=FALSE}
# show data
txt_output %>%
...
```{r pconv06, echo=FALSE, eval = T, message=FALSE, warning=FALSE}
# show data
txts %>%
    substr(start = 1, stop = 1000) %>%
    as.data.frame() %>%
...
```{r tesout1, echo=FALSE, eval = T, message=FALSE, warning=FALSE}
# show data
ocrs %>%
    substr(start = 1, stop = 1000) %>%
    as.data.frame() %>%
...
```{r tesout2, echo=FALSE, eval = T, message=FALSE, warning=FALSE}
# show data
tokens_ocr %>%
    substr(start = 1, stop = 1000) %>%
    as.data.frame() %>%
...
```{r tesout3, echo=FALSE, eval = T, message=FALSE, warning=FALSE}
# show data
clean_ocrtext %>%
    substr(start = 1, stop = 1000) %>%
    as.data.frame() %>%
...
Schweinberger, Martin. 2025. *Converting PDFs to txt files with R*. Brisbane: The University of Queensland. url: https://ladal.edu.au/tutorials/pdf2txt/pdf2txt.html (Version 2025.04.02).


```
@manual{schweinberger2025pdf2txt,

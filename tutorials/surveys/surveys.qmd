...
The entire R Notebook for the tutorial can be downloaded [**here**](tutorials/surveys/surveys.Rmd).  If you want to render the R Notebook on your machine, i.e. knitting the document to html or a pdf, you need to make sure that you have R and RStudio installed and you also need to download the [**bibliography file**](tutorials/surveys/bibliography.bib) and store it in the same folder where you store the Rmd file. <br></p>
...
# load data
ldat <- base::readRDS(url("tutorials/surveys/data/lid.rda", "rb"))
```
...
```{r likert1}
sdat <- base::readRDS(url("tutorials/surveys/data/sdd.rda", "rb"))
```
...
# load data
surveydata <- base::readRDS(url("tutorials/surveys/data/sud.rda", "rb"))
```
...
# load data
ordata <- base::readRDS(url("tutorials/surveys/data/oda.rda", "rb"))
# inspect data
str(ordata)
```

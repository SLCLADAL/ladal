...
<p style='margin-top:1em; text-align:center'>
Click [**here**](https://ladal.edu.au/content/table.Rmd)^[If you want to render the R Notebook on your machine, i.e. knitting the document to html or a pdf, you need to make sure that you have R and RStudio installed and you also need to download the [**bibliography file**](tutorials/table/bibliography.bib) and store it in the same folder where you store the Rmd file.] to download the **entire R Notebook** for this tutorial.<br><br>
...
mytable <- base::readRDS(url("tutorials/table/data/mld.rda", "rb"))
```
...
# load new data
newdata <- base::readRDS(url("tutorials/table/data/mld.rda", "rb"))
```
...
pipeddata <- base::readRDS(url("tutorials/table/data/mld.rda", "rb")) %>%

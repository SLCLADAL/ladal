...
This showcase is a companion to the main [Conceptual Maps tutorial](/tutorials/conceptualmaps_showcase2/conceptualmaps_showcase2.html), which introduces the core concepts. The focus here is on **comparing methods**: understanding what each layout algorithm reveals, and which works best for which purpose.
:::
...
```{r set-data-path, message=FALSE, warning=FALSE}
# Path to the corpus file — adjust if your project structure differs
corpus_file <- "tutorials/conceptualmaps_showcase2/data/ALL_byperiod_nomarkup.txt"
```
...
```{r prep-corpus, eval=TRUE, message=FALSE, warning=FALSE}
prep_word2vec(
  origin      = corpus_file,
...
```{r train-word2vec, eval=TRUE, message=FALSE, warning=FALSE}
...
```{r test-convict, message=FALSE, warning=FALSE}
...
```{r test-letter, message=FALSE, warning=FALSE}
...
```{r test-england, message=FALSE, warning=FALSE}
...
```{r test-australia, message=FALSE, warning=FALSE}
closest_to(training, "australia", 30)
```

```{r test-government, message=FALSE, warning=FALSE}
...
```{r test-period1, message=FALSE, warning=FALSE}
...
```{r test-period2, message=FALSE, warning=FALSE}
...
```{r test-period3, message=FALSE, warning=FALSE}
...
```{r test-period4, message=FALSE, warning=FALSE}
...
```{r word-list, message=FALSE, warning=FALSE}
word_list <- rownames(training)[1:1000]
```

## Subsetting the model {-}

```{r subset-model, message=FALSE, warning=FALSE}
...
```{r cosine-matrix, message=FALSE, warning=FALSE}
...
```{r save-matrix, eval=TRUE, message=FALSE, warning=FALSE}
write.csv(
...
```{r long-form, message=FALSE, warning=FALSE}
...
```{r filter-matrix, message=FALSE, warning=FALSE}
...
```{r build-graph, message=FALSE, warning=FALSE}
...
```{r export-graph, eval=TRUE, message=FALSE, warning=FALSE}
write_graph(
  g,
...
```{r tsne-interactive, message=FALSE, warning=FALSE}
...
```{r tsne-save, eval=TRUE, message=FALSE, warning=FALSE}
...
saveWidget(
  ...
  file   = "tutorials/conceptualmaps_showcase2/data/tsne_cooee.html"
)
```
...
```{r fr-pdf, eval=FALSE, message=FALSE, warning=FALSE}
...
```{r drl-pdf, eval=FALSE, message=FALSE, warning=FALSE}
...
```{r fa2-pdf, eval=FALSE, message=FALSE, warning=FALSE}
...
```{r umap-pdf, eval=FALSE, message=FALSE, warning=FALSE}
...
```{r load-gml, message=FALSE, warning=FALSE}
gml_file <- "tutorials/conceptualmaps_showcase2/data/ALL_byperiod_momarkup3_t400-s10.gml"
g_tp <- read_graph(gml_file, format = "gml")
```

## Visualising with Fruchterman-Reingold {-}

```{r textplot-fr, fig.dim=c(12, 12), message=FALSE, warning=FALSE}
set.seed(1)
plot.igraph(
  g_tp,
...
```{r textplot-fa2, fig.dim=c(12, 12), message=FALSE, warning=FALSE}
...
```{r textplot-fa2-pdf, eval=TRUE, message=FALSE, warning=FALSE}

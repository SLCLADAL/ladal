...
- [Introduction to Data Visualization](/tutorials/introviz/introviz.html) — `ggplot2` fundamentals
- [Introduction to Text Analysis](/tutorials/introta/introta.html) — basic corpus concepts

Familiarity with [Network Analysis](/tutorials/net/net.html) is helpful but not required.
:::
...
domain_lookup <- tibble::tibble(
  word   = target_words,
  domain = c(rep("Emotion", 13), rep("Social", 11), rep("Character", 6))
)
...
set.seed(2024)
mds_result <- smacof::smacof(ppmi_dist, ndim = 2, verbose = FALSE)
...
**Community detection** algorithms identify clusters of densely interconnected nodes — lexical fields in a semantic map. We use the Louvain algorithm [@blondel2008louvain], which is fast and performs well on weighted graphs.
...
This tutorial was written with the assistance of **Claude** (claude.ai), a large language model created by Anthropic. Claude was used to draft the tutorial structure, all explanatory text, all R code chunks, callouts, quiz questions and answers, tables, and the reproducibility checklist. All content was reviewed and is the responsibility of the named author.
:::

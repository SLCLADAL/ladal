...
description: "This showcase tutorial compares six semantic network layout algorithms applied to the COOEE corpus of Australian historical letters, including t-SNE, igraph Fruchterman-Reingold, igraph DRL, ForceAtlas2, UMAP, and textplot GML, using a word2vec semantic space trained with wordVectors. It is aimed at researchers in corpus linguistics and computational semantics who want to understand the strengths and limitations of different approaches to visualising semantic structure."
...
params:
  ...
  url:         "https://ladal.edu.au/tutorials/conceptual_maps_comparison/conceptual_maps_comparison.html"
  institution: "The Language Technology and Data Analysis Laboratory (LADAL), The University of Queensland, Australia"
  description:       "This showcase tutorial compares six semantic network layout algorithms applied to the COOEE corpus of Australian historical letters, including t-SNE, igraph Fruchterman-Reingold, igraph DRL, ForceAtlas2, UMAP, and textplot GML, using a word2vec semantic space trained with wordVectors. It is aimed at researchers in corpus linguistics and computational semantics who want to understand the strengths and limitations of different approaches to visualising semantic structure."
  keywords:       "conceptual maps, semantic networks, t-SNE, UMAP, word embeddings, network layout, corpus linguistics, computational semantics"
  doi:         "10.5281/zenodo.19332090"
---
...
'  "datePublished": "', params$date_published, '",
  ...
  "author": {
    "@type": "Person",
    "name": "', params$author, '",
    "sameAs": "https://orcid.org/0000-0002-1905-6237"
  },
  "publisher": {
...
This showcase is a companion to the main [Conceptual Maps tutorial](/tutorials/conceptual_maps_comparison/conceptual_maps_comparison.html), which introduces the core concepts. The focus here is on **comparing methods**: understanding what each layout algorithm reveals, and which works best for which purpose.
:::
...
- [Getting Started with R](/tutorials/r_intro/r_intro.html) — R basics
- [Word Embeddings and Vector Semantics](/tutorials/embeddings/embeddings.html) — how word2vec works
- [Network Analysis](/tutorials/network_analysis/network_analysis.html) — igraph basics
:::
...
The COOEE corpus file (`ALL_byperiod_nomarkup.txt`) must be present in `tutorials/conceptual_maps_comparison/data/` before running any of the code in this tutorial. Download it using the code below on first use.
:::

```{r download-cooee, eval=FALSE, message=FALSE, warning=FALSE}
# Create the data folder if it does not exist
#dir.create("tutorials/conceptual_maps_comparison/data", recursive = TRUE, showWarnings = FALSE)

# Download the COOEE corpus (run once)
download.file(
  url      = "https://ladal.edu.au/tutorials/conceptual_maps_comparison/data/ALL_byperiod_nomarkup.txt",
  destfile = "tutorials/conceptual_maps_comparison/data/ALL_byperiod_nomarkup.txt",
  mode     = "wb"
)
```
...
corpus_file <- here::here("tutorials/conceptual_maps_comparison/data/ALL_byperiod_nomarkup.txt")
```
...
prep_word2vec(
  origin      = corpus_file,
  destination = here::here("tutorials/conceptual_maps_comparison/data/ALL_byperiod_nomarkup_out.txt"),
  lowercase   = TRUE
)
```
...
training <- train_word2vec(
  train_file  = here::here("tutorials/conceptual_maps_comparison/data/ALL_byperiod_nomarkup_out.txt"),
  output_file = here::here("tutorials/conceptual_maps_comparison/data/ALL_byperiod_nomarkup_w10.bin"),
  threads     = 4,
  vectors     = 200,
  window      = 10,
  force       = TRUE
...
model_file <- here::here("tutorials/conceptual_maps_comparison/data/ALL_byperiod_nomarkup_w10.bin")
...
write.csv(
  ...
  here::here("tutorials/conceptual_maps_comparison/data/word_similarity_matrix_top1000_w10.csv")
)
```
...
write_graph(
  g,
  here::here("tutorials/conceptual_maps_comparison/data/COOEE_w10_simgt0.25.gml"),
  format = "gml"
)
```
...
saveWidget(
  ...
  file   = here::here("tutorials/conceptual_maps_comparison/data/tsne_cooee.html")
)
```
...
```{r fr-basic, eval = FALSE, fig.dim=c(10, 10), message=FALSE, warning=FALSE}
set.seed(1)
plot.igraph(
  g,
  vertex.size      = 0,
...
```{r fr-weighted, eval = FALSE, fig.dim=c(12, 12), message=FALSE, warning=FALSE}
set.seed(1)
plot.igraph(
  g,
...
```{r fr-rescaled, eval = FALSE, fig.dim=c(12, 12), message=FALSE, warning=FALSE}
...
```{r drl, eval = FALSE, fig.dim=c(12, 12), message=FALSE, warning=FALSE}
...
```{r fa2-layout, eval = FALSE, message=FALSE, warning=FALSE}
set.seed(1)
...
```{r fa2-plot, eval = FALSE, fig.dim=c(12, 12), message=FALSE, warning=FALSE}
set.seed(1)
plot.igraph(
  g2,
  layout           = fa2_layout,
...
```{r umap-plot, eval = FALSE, fig.dim=c(12, 12), message=FALSE, warning=FALSE}
set.seed(1)
umap_result <- umap(
  sub_model,
...
download.file(
  url      = "https://ladal.edu.au/tutorials/conceptual_maps_comparison/data/ALL_byperiod_momarkup3_t400-s10.gml",
  destfile = "tutorials/conceptual_maps_comparison/data/ALL_byperiod_momarkup3_t400-s10.gml",
  mode     = "wb"
)
```
...
gml_file <- here::here("tutorials/conceptual_maps_comparison/data/ALL_byperiod_momarkup3_t400-s10.gml")

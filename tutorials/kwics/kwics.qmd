...
Click [**here**](`r binder_url`) to open a notebook-based tool <br>that allows you to upload your own text(s), perform concordancing on them, and download the resulting kwic(s).
:::
...
Concordancing is a fundamental tool in language sciences, involving the extraction of words from a given text or texts [@lindquist2009corpus 5]. Typically, the extracted words or phrases are displayed through keyword-in-context displays (KWICs), where the search term, also referred to as the *node word*, is showcased within its surrounding context, containing both preceding and following words.

Concordancing serves as a cornerstone for many analyses involving text(s), often being the initial step leading towards more detailed examinations of language data [@stafanowitsch2020corpus]. The significance of concordancing lies in its capacity to provide insights into how words or phrases are used, their frequency of occurrence, and the contexts in which they appear. By making it possible to examine a word's or a phrase's contextual use and offering frequency data, concordances empower researchers to better understand usag patterns, surrounding words (collocations), or the collocational profiles of words and phrases [@stafanowitsch2020corpus 50-51]. 

## Examples of concordancing applications{-}

Finding and extracting words from text (concordancing) has a broad avriety of uses. Below, we provide some examples of how and why concordancing can be useful as a method to probe text(s).

1. **Linguistic Research**: Linguists often use concordances to explore the usage patterns of specific words or phrases in different contexts. For instance, researchers might use concordancing to analyse the various meanings and connotations of a polysemous word across different genres of literature.
...
4. **Translation Studies**: Concordancing is also applied in translation studies to analyse the usage of specific terms or expressions in source and target languages. Translators use concordances to identify appropriate equivalents and ensure accurate translation of idiomatic expressions and collocations.
...
![](/images/AntConcConcordance.png){ width=100% } 


There are many great software packages that can be used to create concordances. These include:
...
In addition, many corpora that are available online, such as the [BYU corpora](https://corpus.byu.edu/overview.asp), can be accessed via web interfaces that have in-built concordancing functions.  
  


![](/images/KwicCocaLanguage.png){ width=100% }
...
* **Version Control**: R offers robust version control measures, enabling researchers to track and trace specific versions of software and packages used in their analysis. This ensures that analyses are conducted using consistent software environments, minimising the risk of discrepancies or errors due to software updates or changes.
...
As indicated above, this practical part of the tutorial is conducted within the R environment. If you're new to R or haven't installed it yet, you can find an introduction to R and further instructions on how to use it [here](/tutorials/intror/intror.html). To ensure smooth execution of the scripts provided in this tutorial, you'll need to install some packages from the R library. To install the necessary packages, simply execute the following code. This may take up to 5 minutes, so don't worry if it takes a while.
...
text %>%
  ...
  flextable::fontsize(size = 12, part = "header") %>%
  flextable::align_text_col(align = "center") %>%
  flextable::set_caption(caption = "First 1000 characters of the example text.") %>%
  flextable::border_outer()
```
...
<span>
<p style='margin-top:1em; text-align:center'>
<b>ADDITIONAL INFO</b>: the regular expression ".*CHAPTER I\\." can be interpreted as follows:<br> Match any sequence of characters (.*) followed by the exact text "CHAPTER I" and ending with a period (.). This pattern is commonly used to locate occurrences of a chapter heading labelled "CHAPTER I" within a larger body of text.
</p></span>
</div>
...
mykwic %>%
  ...
  flextable::fontsize(size = 12, part = "header") %>%
  flextable::align_text_col(align = "center") %>%
  flextable::set_caption(caption = "First 10 concordances for the node word Alice in our example text.") %>%
  flextable::border_outer()
```
...
The results indicate that there are `r length(mykwic$keyword)` instances of the search term ("alice"). Moreover, we can also explore how often different variants of the search term were found using the table function. This may be particularly useful for searches involving various search terms (although less so in the present example). 
...
mykwic_long %>%
  ...
  flextable::fontsize(size = 12, part = "header") %>%
  flextable::align_text_col(align = "center") %>%
  flextable::set_caption(caption = "First 10 concordances for the node word *alice* in the example text with extended context (10 elements).") %>%
  flextable::border_outer()
```
...
```{r regex05, echo=FALSE, eval = T, message=FALSE, warning=FALSE}
...
```{r regex05b, echo=FALSE, message=FALSE, warning=FALSE, class.source='klippy'}
# inspect data
df_regex %>%
  as.data.frame() %>%
  flextable::flextable() %>%
...
```{r regex03, echo=FALSE, eval = T, message=FALSE, warning=FALSE}
...
```{r regex07b, echo=FALSE, message=FALSE, warning=FALSE, class.source='klippy'}
# inspect data
df_regex %>%
  as.data.frame() %>%
  flextable::flextable() %>%
...
```{r regex09, echo=FALSE, eval = T, message=FALSE, warning=FALSE}
symbols3 <- c(
...
```{r regex09b, echo=FALSE, message=FALSE, warning=FALSE, class.source='klippy'}
# inspect data
df_regex %>%
  as.data.frame() %>%
  flextable::flextable() %>%
...
```{r orderkwic4h, echo=FALSE, message=FALSE, warning=FALSE, class.source='klippy'}
# inspect data
mykwic_following %>%
  head(10) %>%
  flextable::flextable() %>%
...
```{r customkwic8b, echo=FALSE, message=FALSE, warning=FALSE, class.source='klippy'}
# inspect data
mykwic_pooralice %>%
  as.data.frame() %>%
  head() %>%

...
<p style='margin-top:1em; text-align:center'>
Click [**here**](https://ladal.edu.au/content/string.Rmd)^[If you want to render the R Notebook on your machine, i.e. knitting the document to html or a pdf, you need to make sure that you have R and RStudio installed and you also need to download the [**bibliography file**](tutorials/string/bibliography.bib) and store it in the same folder where you store the Rmd file.] to download the **entire R Notebook** for this tutorial.</p><br>
...
# read in text
exampletext <- base::readRDS(url("tutorials/string/data/tx1.rda", "rb"))
# inspect
exampletext
```
...
# read in text
splitexampletext <- base::readRDS(url("tutorials/string/data/tx2.rda", "rb"))
# inspect
splitexampletext
```
...
additionaltext <- base::readRDS(url("tutorials/string/data/tx3.rda", "rb"))
# inspect
additionaltext
```
...
sentences <- base::readRDS(url("tutorials/string/data/tx4.rda", "rb"))
# inspect
sentences
```

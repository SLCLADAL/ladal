...
params:
  ...
  url:         "https://ladal.edu.au/tutorials/descriptive_stats/descriptive_stats.html"
  institution: "The Language Technology and Data Analysis Laboratory (LADAL), The University of Queensland, Australia"
  description: "This tutorial introduces descriptive statistics in R, covering measures of central tendency (mean, median, mode), measures of dispersion (variance, standard deviation), creating summary statistics, exploring data distributions, and identifying outliers. It is the recommended starting point for the statistics section and is aimed at researchers in linguistics and the humanities with no prior statistical experience."
  keywords:       "descriptive statistics, mean, median, variance, standard deviation, data distribution, outliers, summary statistics"
  doi:         "10.5281/zenodo.19332863"
---
...
Or compute the interval by hand from the formula. This makes explicit what `CI()` and `t.test()` do internally: the mean plus and minus a *t*-quantile times the standard error.  
  
```{r ds37, message=FALSE, warning=FALSE}  
n      <- length(rts$RT)  
xbar   <- mean(rts$RT)  
se     <- sd(rts$RT) / sqrt(n)  
t_crit <- qt(0.975, df = n - 1)   # two-sided 95% -> 0.975 quantile  
...
The simplest way to see how this works is to do it by hand with base R: draw 1,000 samples *with replacement* from the data, compute the mean of each, and take the 2.5th and 97.5th percentiles of those 1,000 means as the interval.  
  
```{r ds38, message=FALSE, warning=FALSE}  
set.seed(42)  
  
# 1,000 bootstrap resamples of the mean  
boot_means <- replicate(1000, mean(sample(rts$RT, replace = TRUE)))  
  
# bootstrapped mean and 95% percentile CI  
mean(boot_means)  
...
The `boot` package does the same thing more flexibly and offers several methods for turning the resampled statistics into an interval (`boot.ci()` reports normal, basic, percentile, and studentised intervals):  
...
`binom.test()` returns the exact Clopper-Pearson interval, which is conservative (wider than it needs to be) for small samples. An alternative that is generally preferred is the **Wilson score interval**, which `stats::prop.test()` returns when the continuity correction is switched off:  
...
stats::prop.test(  
  ...
  n          = 20,  
  conf.level = 0.95,  
  correct    = FALSE   # FALSE gives the Wilson score interval  
)$conf.int  
```  
...
When there are more than two nominal categories (e.g., which of several linguistic constructions was chosen), the proportions are not independent — they must sum to 1 — so we want **simultaneous** confidence intervals that hold for all categories at once. Base R has no ready-made function for this, but the widely used method proposed by Goodman (1965) is short enough to implement directly. It is based on the chi-square distribution with *k* − 1 degrees of freedom, where *k* is the number of categories:  
...
# Goodman (1965) simultaneous 95% CIs  
q     <- qchisq(0.95, df = k - 1)  
root  <- sqrt(q * (q + 4 * observed * (n - observed) / n))  
lower <- (q + 2 * observed - root) / (2 * (n + q))  
upper <- (q + 2 * observed + root) / (2 * (n + q))  
  
data.frame(  
  category   = paste0("Category ", seq_len(k)),  
  count      = observed,  
  proportion = round(observed / n, 3),  
  lower      = round(lower, 3),  
  upper      = round(upper, 3)  
)  
```  
  
Each interval is wider than the corresponding single-proportion interval would be, because the method guarantees that *all four* intervals contain their true proportions simultaneously with 95% confidence.  
...
# all three distributions scaled to mean 0, variance 1 so that only
# tail weight and peakedness differ
s_t   <- sqrt(3)            # t(df=3) has variance 3 -> divide by sqrt(3)
a     <- sqrt(5)            # Beta(2,2) rescaled to [-a, a] has variance a^2/5 = 1

y_meso  <- dnorm(x_vals, 0, 1)
y_lepto <- dt(x_vals * s_t, df = 3) * s_t
y_platy <- dbeta((x_vals + a) / (2 * a), 2, 2) / (2 * a)   # 0 outside [-a, a]

type_levels <- c("Mesokurtic (normal)\nExcess kurtosis = 0",
                 "Leptokurtic (t-dist., df=3)\nExcess kurtosis > 0",
                 "Platykurtic (scaled Beta(2,2))\nExcess kurtosis < 0")

df_kurt <- data.frame(
  x    = rep(x_vals, 3),
  y    = c(y_meso, y_lepto, y_platy),
  type = factor(rep(type_levels, each = length(x_vals)), levels = type_levels)
)
...
df_kurt |>
  ...
  labs(title = "Kurtosis: mesokurtic, leptokurtic, and platykurtic distributions",
       subtitle = "All three distributions have mean 0 and variance 1",
...
Winsorisation needs no special package. We find the 5th and 95th percentiles with `quantile()` and then clamp the data to that range: `pmax()` raises every value below the lower cut-off to the cut-off, and `pmin()` lowers every value above the upper cut-off.  
  
```{r wins1, message=FALSE, warning=FALSE}  
# 5th and 95th percentiles as cut-offs  
cutoffs <- quantile(rt_skewed, probs = c(0.05, 0.95))  
  
# Winsorise: clamp values to the cut-offs  
rt_winsorised <- pmin(pmax(rt_skewed, cutoffs[1]), cutoffs[2])  
...
This tutorial was revised and substantially expanded with the assistance of **Claude** (claude.ai), a large language model created by Anthropic. All content was reviewed, edited, and approved by the author (Martin Schweinberger), who takes full responsibility for its accuracy.
:::

...
The entire R Notebook for the tutorial can be downloaded [here](/tutorials/tree/tree.qmd).  If you want to render the R Notebook on your machine, i.e. knitting the document to html or a pdf, you need to make sure that you have R and RStudio installed and you also need to download the [**bibliography file**](/assets/bibliography.bib) and store it in the same folder where you store the Rmd file.
</p>
...
notebook_path <- "interactive-notebooks/notebooks/tree_cb.ipynb"
...
```{r cit13, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r cit24, eval = T, echo=T, message=FALSE, warning=FALSE}
# set.seed
set.seed(111)
# calculate tree1
dtree <- tree::tree(LikeUser ~ Age + Gender + Status, data = citdata, split = "gini")
...
```{r citn1, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r citn2, echo=FALSE, message=FALSE, warning=FALSE}
citdata2 <- citdata2 %>%
    arrange(Age)
```
...
```{r citn3, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r citn5, echo=FALSE, message=FALSE, warning=FALSE}
...
citdata <- read.delim("tutorials/tree/data/treedata.txt",
    header = T, sep = "\t"
)
citdata <- citdata %>%
...
```{r rf2, echo=FALSE, message=FALSE, warning=FALSE}
...
Schweinberger, Martin. `r format(Sys.time(), '%Y')`. *Tree-Based Models in R*. Brisbane: The University of Queensland. url: https://ladal.edu.au/tutorials/tree.html (Version `r format(Sys.time(), '%Y.%m.%d')`).

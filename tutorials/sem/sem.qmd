...
```{r install, eval = F, echo=F, message=F, warning=F}
renv::install("semPlot", type = "binary")
renv::install("semTools", type = "binary")
```
...
```{r loadpkgs, message=F, warning=F}
# load packages
...
```{r simulate_data, message=F, warning=F}
set.seed(42)
n <- 300
...
```{r viewdata, echo=F, message=F, warning=F}
semdata %>%
  head(10) %>%
  flextable() %>%
...
```{r corr_viz, message=F, warning=F}
...
```{r cfa_spec, message=F, warning=F}
cfa_model <- '
  # Measurement model
  ANX =~ anx1 + anx2 + anx3
  EFF =~ eff1 + eff2 + eff3
...
```{r cfa_loadings, message=F, warning=F}
...
Cut-offs are from @hu1999cutoff. Note that these are guidelines, not hard thresholds. Model fit must always be evaluated in the context of model complexity and sample size.
...
```{r cfa_fitindices, message=F, warning=F}
...
```{r reliability, message=F, warning=F}
...
```{r cfa_plot, message=F, warning=F, fig.width=9, fig.height=6}
semPlot::semPaths(
  cfa_fit,
...
```{r sem_spec, message=F, warning=F}
sem_model <- '
  # --- Measurement model ---
  ANX =~ anx1 + anx2 + anx3
  EFF =~ eff1 + eff2 + eff3
...
```{r sem_paths, message=F, warning=F}
...
```{r sem_plot, message=F, warning=F, fig.width=10, fig.height=7}
semPlot::semPaths(
  sem_fit,
  what       = "std",
  layout     = "tree2",
...
```{r med_spec, message=F, warning=F}
mediation_model <- '
  # --- Measurement model ---
  ANX =~ anx1 + anx2 + anx3
  EFF =~ eff1 + eff2 + eff3
...
```{r med_fit, message=F, warning=F, cache=TRUE}
set.seed(42)
...
1. **Path *a*** (*EFF* → *MOT*): Is Self-Efficacy a significant predictor of Motivation?
2. **Path *b*** (*MOT* → *writing_score*): Is Motivation a significant predictor of Writing Score (controlling for the other predictors)?
3. **Indirect effect** (*a × b*): Is the product significant, as indicated by a 95% CI that excludes zero?
4. **Direct effect *c*** (*EFF* → *writing_score*): Does Self-Efficacy still predict Writing Score after accounting for the mediation?
...
```{r nested_mod, message=F, warning=F}
constrained_model <- '
  # --- Measurement model ---
  ANX =~ anx1 + anx2 + anx3
  EFF =~ eff1 + eff2 + eff3
...
```{r mod_indices, message=F, warning=F}
...
check_question(
  ...
  random_answer_order = TRUE,
  button_label = "Check answer",
  right  = "Correct! When Model A is nested within Model B (A has more constraints / fewer free parameters), the Δχ² = χ²(A) − χ²(B) follows a chi-square distribution with degrees of freedom equal to the difference in df between the two models. A significant result (p < .05) means the extra parameters in Model B account for a statistically significant improvement in fit — the simpler model does not fit as well. A non-significant result supports the simpler (more parsimonious) model.",
  wrong  = "Think about what it means for a more constrained model to have a higher chi-square. A significant Δχ² means the constraints imposed in the simpler model cause a significant deterioration in fit. What does that imply about those constraints?"
)
```
...
Following current best practice [@kline2023principles; @jackson2009reporting]:
...
colnames(workflow_df) <- c("Step", "Action", "Key R function(s)")

...
8. **Key `stringr` functions** — `str_detect()`, `str_extract()`, `str_replace()`, and more  
...
data.frame(  
  ...
  dplyr::rename("Sequence (in R code)" = Sequence,  
                "Example (R string)" = Example_R) |>  
  flextable() |>  
  ...
  flextable::fontsize(size = 11, part = "header") |>  
  flextable::align_text_col(align = "left") |>  
  flextable::set_caption(caption = "Special escape sequences for common character classes.") |>  
  flextable::border_outer()  
```  
...
check_question("\\\\w*\\\\d\\\\w* — a word character sequence containing at least one digit",  
               ...
               random_answer_order = TRUE,  
               button_label = "Check answer",  
               right = "Correct! \\w*\\d\\w* matches: zero or more word characters (\\w*), then at least one digit (\\d), then zero or more word characters (\\w*). This captures tokens like 'A4', 'mp3', 'COVID-19' (hyphen aside — you may need \\w*\\d\\w*(-\\w*\\d\\w*)* for hyphenated forms). The plain \\d+ would extract only the digit sequence, not the surrounding letters. Wrapping in \\b...\\b adds whole-word anchoring.",  
...
check_question("(?<=@)[\\w.]+(?=\\.\\w+$) — a lookbehind for @ and lookahead for the final .extension",  
               ...
               random_answer_order = TRUE,  
               button_label = "Check answer",  
               right = "Correct! (?<=@) is a positive lookbehind that requires @ immediately before the match. [\\w.]+ matches word characters and dots (the full domain like 'uq.edu'). (?=\\.\\w+$) is a positive lookahead requiring a dot followed by word characters at the end of the string ('.au'). The lookbehind and lookahead ensure we are positioned correctly without including @ or the final extension in the match. This is a deliberately tricky extraction — in practice you would often split on @ first, then extract the relevant part of the domain.",  
...
check_question("It swaps the two words on either side of 'and', using backreferences \\\\1 and \\\\2 to refer to the captured groups",  
               ...
               random_answer_order = TRUE,  
               button_label = "Check answer",  
               right = "Correct! The pattern (\\w+) and (\\w+) captures two words in groups 1 and 2. The replacement \\2 and \\1 puts them back in reverse order. So 'cats and dogs' becomes 'dogs and cats'. Backreferences in str_replace_all() replacements use \\1, \\2, etc. to refer to what was captured in the corresponding group. This is a powerful technique for reordering, reformatting, and transforming structured text.",  
...
```{r exercise_setup, echo=FALSE}  
# Example tokenised text for all exercises below  
text <- c(  
...
check_question('\\\\bwalk\\\\w*\\\\b — word boundary + "walk" + zero or more word characters',  
               ...
               random_answer_order = TRUE,  
               button_label = "Check answer",  
               right = 'Correct! \\bwalk\\w*\\b matches the literal string "walk" at a word boundary, followed by zero or more word characters (\\w*), up to the next word boundary. This captures walk, walks, walked, walking, walker, and any other form beginning with "walk". The word boundaries prevent matching inside longer words like "sidewalk" if that is not desired — remove them if you want substring matches too. The plain "walk" or "walk.*" without boundaries would work on a tokenised vector since each token is one word, but \\bwalk\\w*\\b is the most explicit and transferable form.',  
...
check_question('\\\\b[Uu]n\\\\w+ — word boundary + "un" (case-flexible) + one or more word characters',  
               ...
               random_answer_order = TRUE,  
               button_label = "Check answer",  
               right = 'Correct! \\b[Uu]n\\w+ matches: a word boundary (\\b), then "un" or "Un", then one or more word characters (\\w+). The \\w+ ensures we match a word that continues beyond "un" — this avoids matching the word "un" alone as a token. The [Uu] handles both "ungrammatical" and "Unknown". On a tokenised vector where each element is one word, "^un" also works well, but \\b[Uu]n\\w+ generalises to running on raw text too.',  
...
check_question('\\\\b\\\\d+\\\\b — word boundaries around one or more digits',  
               ...
               random_answer_order = TRUE,  
               button_label = "Check answer",  
               right = 'Correct! \\b\\d+\\b matches one or more digits (\\d+) as a complete token (word boundaries on both sides). This matches "2024", "42", "100" but not "COVID-19" or "mp3" (which contain non-digit characters too). Use \\d+ without boundaries if you want to match digit sequences embedded in mixed tokens. The patterns [0-9] and \\d match only a single digit; \\d* matches zero or more digits and would return TRUE for every string (since zero digits is satisfied by any string).',  
...
check_question('\\\\b\\\\w+ing\\\\b — one or more word characters followed by "ing" at a word boundary',  
               ...
               random_answer_order = TRUE,  
               button_label = "Check answer",  
               right = 'Correct! \\b\\w+ing\\b matches: a word boundary, then one or more word characters (\\w+ ensures there is a stem before the suffix), then the literal "ing", then a word boundary. This captures running, walking, thinking, helping, etc. On a tokenised vector, "ing$" (anchoring to string end) works equally well. The pattern ".*ing" would also match but is less precise — on raw text it would match greedily across sentence boundaries. "[ing]" is completely different: it is a character class matching any single character that is i, n, or g.',  
...
check_question('"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\\\.[A-Za-z]{2,}" — local part + @ + domain + TLD',  
               ...
               random_answer_order = TRUE,  
               button_label = "Check answer",  
               right = 'Correct! The full email pattern [A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,} captures: the local part (letters, digits, dots, underscores, %, +, -), the @ symbol, the domain (letters, digits, dots, hyphens), a literal dot, and a top-level domain of at least 2 letters. This handles standard email formats robustly. Simpler patterns like \\w+@\\w+ miss dots and hyphens in domains. \\S+@\\S+ is too broad and would match things like "price@$4" or "not@an-email!". Email validation with regex is notoriously tricky — the pattern above covers the most common real-world cases.',  
...
check_question('"\\\\w*\\\\d\\\\w*" — word characters, then a digit, then word characters',  
               ...
               random_answer_order = TRUE,  
               button_label = "Check answer",  
               right = 'Correct! \\w*\\d\\w* matches zero or more word characters, then at least one digit, then zero or more word characters. This captures mp3 (letters then digit), A4 (letter then digit), and type2 (letters then digit). Note it also matches pure numbers like "2024" — if you want to exclude those, use [a-zA-Z]\\w*\\d|\\d\\w*[a-zA-Z] to require at least one letter AND one digit. \\d+ matches only digit sequences. [a-z]\\d requires a letter immediately before the digit, missing "A4" if case-sensitive. \\b\\d\\b matches only a single digit as a complete token.',  
...
check_question('"\\\\b\\\\w+-\\\\w+\\\\b" — word characters, hyphen, word characters within word boundaries',  
               ...
               random_answer_order = TRUE,  
               button_label = "Check answer",  
               right = 'Correct! \\b\\w+-\\w+\\b matches: a word boundary, one or more word characters (the first element), a literal hyphen, one or more word characters (the second element), a word boundary. This captures well-being, self-aware, long-term, and any other standard hyphenated compound. The plain "-" would match any token with a hyphen, including punctuation. \\w-\\w requires exactly one character on each side, missing multi-character elements. [a-z]-[a-z] is similar but also case-sensitive. For multi-part compounds like "mother-in-law", extend to \\w+(-\\w+)+.',  
...
check_question('"\\\\b[A-Z][a-z]+\\\\b" — uppercase first letter followed by one or more lowercase letters',  
               ...
               random_answer_order = TRUE,  
               button_label = "Check answer",  
               right = 'Correct! \\b[A-Z][a-z]+\\b matches: a word boundary, one uppercase letter, one or more lowercase letters, a word boundary. This matches London, Paris, Grammar, Syntax — title-case words. It deliberately excludes ALL-CAPS tokens like "NLP" or "COVID" (which [A-Z]+ would match) and single uppercase letters. Note: at sentence boundaries, even non-proper nouns are capitalised; for proper-noun detection you would need additional context filtering. On a raw text, use with str_extract_all() rather than on individual tokens.',  
...
check_question('"\\\\?" — matches any token ending in a literal question mark (on a tokenised vector)',  
               ...
               random_answer_order = TRUE,  
               button_label = "Check answer",  
               right = 'Correct! On a tokenised vector (one word/punctuation per element), \\? detects any token containing a literal question mark. Since ? is a quantifier in regex, it must be escaped as \\? to match the literal character. \\?$ is equally correct — it anchors the ? to the end of the string, which is more precise. .*\\?$ works too and is the most explicit form: any characters, then a literal ?, then end of string. The unescaped "?" alone is a regex quantifier meaning "zero or one of the preceding element" — on its own with nothing preceding it, it either errors or matches everything.',  
...
check_question('"[aeiouAEIOU]{2}" — two consecutive vowels (case-insensitive vowel class, quantified)',  
               ...
               random_answer_order = TRUE,  
               button_label = "Check answer",  
               right = 'Correct! [aeiouAEIOU]{2} matches any two consecutive vowels (the same or different: "oo", "ea", "ai", "ou"). This captures agreement (ee? no — "ee" is in "see"; agreement has "ee"? check: a-g-r-e-e-m-e-n-t → yes, "ee"), book (oo), see (ee), moon (oo). Both [aeiouAEIOU]{2} and [aeiou][aeiou] are correct — the {2} form is more compact and easier to extend to {3} or {2,}. [aeiou]+ also matches double vowels but would return TRUE for any vowel at all (single included). (aa|ee|ii|oo|uu) only matches identical doubled vowels, missing "ea", "ou", "ai" etc.',  
...
data.frame(  
  ...
  flextable::fontsize(size = 11, part = "header") |>  
  flextable::align_text_col(align = "left") |>  
  flextable::set_caption(caption = "Quick reference: regular expression patterns in R.") |>  
  flextable::border_outer()  
```  
  
## `stringr` Function Summary {-}  
  
```{r fn_table, echo=FALSE}  
data.frame(  
  Function            = c(  
    "str_detect(x, p)",  
    "str_count(x, p)",  

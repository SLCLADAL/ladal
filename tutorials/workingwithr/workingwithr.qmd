...
This tutorial was revised and restyled with the assistance of **Claude** (claude.ai), a large language model created by Anthropic. Claude was used to align the document with LADAL tutorial conventions: adding Learning Objectives and Prerequisites callouts, section overview callouts, the automated citation system, converting the setup section to LADAL style, replacing `%>%` with the native `|>` pipe throughout, fixing the BibTeX formatting bug (missing comma before `doi`), fixing the broken closing `:::` on the citation callout, removing the duplicate "What This Tutorial Covers" callout, and updating the floating image to `gy_chili.png`. All substantive content — code, explanations, and exercises — was retained from the original. All changes were reviewed and approved by Martin Schweinberger, who takes full responsibility for the tutorial's accuracy.
:::

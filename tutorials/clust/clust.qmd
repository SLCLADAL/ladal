...
<p style='margin-top:1em; text-align:center'>
Click [here](/tutorials/clust/clust.qmd)^[If you want to render the R Notebook on your machine, i.e. knitting the document to html or a pdf, you need to make sure that you have R and RStudio installed and you also need to download the [**bibliography file**](/assets/bibliography.bib and store it in the same folder where you store the Rmd file.] to download the **entire R Notebook** for this tutorial.
</p>
</div>
...
```{r cl12, eval = T, echo=FALSE, message=FALSE, warning=FALSE}
...
legend("topleft",
  inset = .05, title = "", bty = "n", lty = c(3, 2, 4), lwd = 2,
...
# load data
vsmdata <- base::readRDS("tutorials/clust/data/vsd.rda", "rb")
```
...
```{r pca1, eval = F, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r pca2, eval = F, echo=FALSE, message=FALSE, warning=FALSE}
# log transform
log.ir <- log(iris[, 1:4])
ir.species <- iris[, 5]
...
```{r pca3, eval = F, echo=FALSE, message=FALSE, warning=FALSE}
# print method
print(ir.pca)
```


```{r pca4, eval = F, echo=FALSE, message=FALSE, warning=FALSE}
# plot method
plot(ir.pca, type = "l")
```

```{r pca5, eval = F, echo=FALSE, message=FALSE, warning=FALSE}
# summary method
summary(ir.pca)
```

```{r pca6, eval = F, echo=FALSE, message=FALSE, warning=FALSE}
# predict PCs
predict(ir.pca, newdata = tail(log.ir, 2))
```


```{r pca7, eval = F, echo=FALSE, message=FALSE, warning=FALSE}
# load library
library(devtools)
...
```{r pca8, eval = F, echo=FALSE, message=FALSE, warning=FALSE}
require(caret)
...
```{r pca9, eval = F, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r mds1, eval = F, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r mds2, eval = F, echo=FALSE, message=FALSE, warning=FALSE}
# Nonmetric MDS
# N rows (objects) x p columns (variables)
# each row identified by a unique row name
...
Schweinberger, Martin. 2023. *Cluster and Correspondence Analysis in R*. Brisbane: The University of Queensland. url: https://ladal.edu.au/tutorials/clust.html (Version 2023.05.31).

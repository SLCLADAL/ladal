...
# install ggflags to be able to use flag geoms
...
We can now use the [`gglikert`](https://cran.r-project.org/web/packages/ggstats/vignettes/gglikert.html) from the `ggstats` package to visualise the data.

```{r gglikert01}
survey %>%
  # select only columns that represent questions
  dplyr::select(matches("Q"))  %>%
  gglikert() 
```

The `ggplikert` function is interesting becasue it allows us to modify the visualisation quite easily using ggplot-syntax. For example, below, we will only look at the first 4 questions, remove labels, split the data into groups (facets), and use a different color palette.


```{r gglikert02, warning=F, message=F}
survey %>%
  # define that we want to use columns in which 01, 02, 03, or 04 occur in the column header
  gglikert(matches("01|02|03|04"),
           # reduce label size
           labels_size = 3,
           # remove labels
           add_labels = FALSE,
...
# saving
ggsave(here::here("images/my_gglikert_plot.png"), width = 24, height = 12, units = "cm")
```


Alternatively, we can use the `plot` and the `likert` function from the `likert` package to visualize the survey data.
...
When visualizing country-level data, figures can be made more engaging and informative by incorporating national flags. In this tutorial, we demonstrate how to include flags in R-based visualizations using a practical example. Specifically, we will recreate a figure inspired by @schweinberger2025lingua, which illustrates the distribution of vulgar language in online discourse across 20 English-speaking regions.

```{r flags1}
# Create a dataframe with the given data
flagsdf <- data.frame(
  Region = c("Australia", "Bangladesh", "Canada", "Ghana", "Great Britain", 
             "Hong Kong", "India", "Ireland", "Jamaica", "Kenya", "Malaysia", 
             "New Zealand", "Nigeria", "Pakistan", "Philippines", "Singapore",
             "South Africa", "Sri Lanka", "Tanzania", "United States"),
  Percent = c(0.022, 0.007, 0.017, 0.008, 0.025, 0.009, 0.010, 0.019, 0.017, 
              0.011, 0.019, 0.020, 0.016, 0.010, 0.015, 0.021, 0.012, 0.012, 
              0.008, 0.036),
  Kachru = c("Inner circle", "outer circle", "Inner circle", "outer circle", 
             "Inner circle", "outer circle", "outer circle", "Inner circle", 
             "outer circle", "outer circle", "outer circle", "Inner circle", 
             "outer circle", "outer circle", "outer circle", "outer circle", 
             "outer circle", "outer circle", "outer circle", "Inner circle"),
  country = c("au", "bd", "ca", "gh", "gb", "hk", "in", "ie", "jm", "ke", "my",
              "nz", "ng", "pk", "ph", "sg", "za", "lk", "tz", "us"),
...
The dataset consists of 20 observations and includes the following four variables:

* Region: Names of 20 English-speaking regions, such as Australia, Bangladesh, Canada, and 17 others.  
* Percent: Numeric values representing relative frequencies (e.g., of vulgar language use). Summary statistics:n = 20, Mean = 0.020, SD = 0.00712, Median = 0.020, MAD = 0.0074, Range = [0.007, 0.036]
* Kachru: Categorical variable indicating the sociolinguistic classification of English use based on Kachru's model, with two levels:
  * Outer circle (n = 14)
  * Inner circle (n = 6)
* country: Two-letter ISO 3166-1 alpha-2 country codes corresponding to each region, such as au (Australia), bd (Bangladesh), and ca (Canada).

Next, we create a bar graph using `geom_bar()` and add country flags using `geom_flag()` from the `ggflags` package [@ggflags]. Note that `geom_flag()` requires a variable named country containing lowercase ISO 3166-1 alpha-2 codes for each observation.

```{r flags3, warning=F, message=F}
flagsdf %>%
...
clinton <- base::readRDS("tutorials/dviz/data/Clinton.rda", "rb") %>% paste0(collapse = " ")
# load and process speeches by trump
trump <- base::readRDS("tutorials/dviz/data/Trump.rda", "rb") %>% paste0(collapse = " ")
```
...
Schweinberger, Martin. 2025. *Data Visualization with R*. Brisbane: The University of Queensland. url: https://ladal.edu.au/tutorials/dviz/dviz.html (Version 2025.08.05).

```
@manual{schweinberger2025dviz,

...
The `ggplikert` function is interesting becasue it allows us to modify the visualisation quite easily using ggplot-syntax. For example, below, we will use facets and use a different color palette.


```{r gglikert02}
survey %>%
  # select only columns that represent questions or groups
  dplyr::select(matches("Q|G"))  %>%
  # define that we want to use all columns with Q for visialisation
  gglikert(matches("Q"),
           # define that we want to group by the Group column

...
<p style='margin-top:1em; text-align:center'>
Click [**here**](https://ladal.edu.au/content/dviz.Rmd)^[If you want to render the R Notebook on your machine, i.e. knitting the document to html or a pdf, you need to make sure that you have R and RStudio installed and you also need to download the [**bibliography file**](tutorials/dviz/bibliography.bib) and store it in the same folder where you store the Rmd file.] to download the **entire R Notebook** for this tutorial.<br><br>
...
```{r prep4}
# load data
pdat <- base::readRDS(url("tutorials/dviz/data/pvd.rda", "rb"))
```
...
```{r llik_01}
ldat <- base::readRDS(url("tutorials/dviz/data/lid.rda", "rb"))
```
...
```{r likert1}
sdat <- base::readRDS(url("tutorials/dviz/data/sdd.rda", "rb"))
```
...
clinton <- base::readRDS(url("tutorials/dviz/data/Clinton.rda", "rb")) %>% paste0(collapse = " ")
# load and process speeches by trump
trump <- base::readRDS(url("tutorials/dviz/data/Trump.rda", "rb")) %>% paste0(collapse = " ")
```

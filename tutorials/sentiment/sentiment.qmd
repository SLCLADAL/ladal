...
The entire R Notebook for the tutorial can be downloaded [**here**](tutorials/sentiment/sentiment.Rmd).  If you want to render the R Notebook on your machine, i.e. knitting the document to html or a pdf, you need to make sure that you have R and RStudio installed and you also need to download the [**bibliography file**](tutorials/sentiment/bibliography.bib) and store it in the same folder where you store the Rmd file. <br><br>
...
darwin <- base::readRDS(url("tutorials/sentiment/data/origindarwin.rda", "rb"))
twain <- base::readRDS(url("tutorials/sentiment/data/twainhuckfinn.rda", "rb"))
orwell <- base::readRDS(url("tutorials/sentiment/data/orwell.rda", "rb"))
lovecraft <- base::readRDS(url("tutorials/sentiment/data/lovecraftcolor.rda", "rb"))
```
